    ...
    for (int i = 0; i < vertex_count; i++) {
        if (!is_vertex_connected_to_own_group(i)) {
            ...
            for (int g = 0; g < parts; g++) {
                if (g != current && group_sizes[g] < max_size && has_connection_in_group(i, g)) {
                    best_target = g;
                    break;
                }
            }
...
void fix_group_connectivity(int vertex_count, int allowed_balance_diff) {
    int *group_sizes = calloc(2, sizeof(int));
    if(group_sizes == NULL) {
        printf("Blad pamieci");
        exit(15);
    }
    ...
    for (int i = 0; i < vertex_count; i++) {
        if (!is_vertex_connected_to_own_group(i)) {
            ...
            if (has_connection_in_group(i, target_group)) {
                if ((target_group == 0 && group_sizes[0] + 1 <= group_sizes[1] - 1 + allowed_balance_diff) ||
                    (target_group == 1 && group_sizes[1] + 1 <= group_sizes[0] - 1 + allowed_balance_diff)) {
                    vertices[i].group = target_group;
                    group_sizes[current_group]--;
                    group_sizes[target_group]++;
                    }
            }
        }
    }
...
void remove_cross_group_connections(int vertex_count, double error_margin) {
    for (int i = 0; i < vertex_count; i++) {
        ...
        if (new_edge_num == 0 && error_margin != -1) {
            printf("Blad: Podzial nie powiodl sie.");
            exit(16);
        }
...
void validate_graph_data(int max_matrix, int *x_coords, int x_count, int *y_offsets, int y_offsets_count, int *connections, int count_conn, int *offsets, int count_offsets) {
    if (x_count <= 0) {
        printf("Blad: Niepoprawny format pliku wejsciowego.");
        exit(13);
    }
...
void flags_error(char **format, char *raw_parts, int *parts, char **method, char *raw_error_margin, double *error_margin) {
    ...
    if(*parts < 2) {
        printf("Blad: liczba czesci musi być wieksza lub rowna 2.");
        exit(10);
    }
